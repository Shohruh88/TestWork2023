...
//                                        [ 'name' => 'gender' ],
                                        // [ 'name' => 'date_of_birth', 'type' => 'date'],
                                         ...
                                         [ 'name' => 'employeed_at', 'type' => 'date'],
                                         [ 'name' => 'employeed_to', 'type' => 'date'],
//                                        [ 'name' => 'url_website', 'type' => 'url' ],
                                    ...
                                    @foreach ($fields_array as $field)
                                        <tr>
                                            ...
                                            @if(!in_array($field_name, ['birthday', 'employeed_to', 'employeed_at']))
...
{{--                                                @endif--}}
{{--                                            </td>--}}
                                            @if ($field_name == 'birthday' && $userprofile->user->$field_name != '')
                                                <th>{{ __("Tug'ilgan sanasi") }}</th>
                                                <td>
                                                    @if(auth()->user()->id == $userprofile->user_id)
                                                    {{ $userprofile->user->$field_name }}
                                                    @else
                                                    {{  $userprofile->user->$field_name->format('jS \\of F') }}
                                                    @endif
                                                </td>
                                            @elseif ($field_name == 'employeed_at' && $userprofile->user->$field_name != '')
                                                <th>{{ __("Otpuskaga chiqish sanasi") }}</th>
                                            ...
                                            @elseif ($field_name == 'employeed_to' && $userprofile->user->$field_name != '')
                                                <th>{{ __("Otpuskadan qaytish sanasi") }}</th>
                                                ...
                                                <th>{{ __("Jami otpuska vaqti") }}</th>

                                                <td>
                                                    @if(auth()->user()->id == $userprofile->user_id)
                                                        @php
                                                            $dateAt = \Carbon\Carbon::parse($userprofile->user->employeed_at);
                                                            $dateTo = \Carbon\Carbon::parse($userprofile->user->employeed_to);

                                                            $diff = $dateAt->diffInDays($dateTo);
                                                        @endphp
                                                        {{ $diff . ' kun'}}
                                                    @endif
                                                </td>
